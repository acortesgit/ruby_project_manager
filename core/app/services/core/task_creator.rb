      ...
      if task.save
        ...
        if assignee
          ...
          begin
            ...
            job_id = job.respond_to?(:job_id) ? job.job_id : 'N/A'
            Rails.logger.info("TaskCreator: NotificationJob enqueued with job_id: #{job_id}")
          rescue => e
            Rails.logger.error("TaskCreator: Error enqueuing notification: #{e.message}")
          end
