...
class TaskStatusUpdater < ApplicationService
  ...
  def call
    ...
    if task.update(status: new_status)
      # Enqueue ActivityLoggerJob to log the change
      # Note: ActivityLoggerJob will be implemented in Phase 2.2
      # For now, we'll create the activity directly
      ...
      # Temporary: Create activity directly until ActivityLoggerJob is implemented
      Activity.create!(
        record: task,
        action: "status_changed",
        user: user,
